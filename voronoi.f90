PROGRAM voronoi
  IMPLICIT NONE
  INTEGER :: num_unitcell_points, num_total_points, dim, stat
  !number of total points includes points in other periodic cells adjacent to the unit cell
  REAL, DIMENSION(:), ALLOCATABLE :: unitcell_dim
  ...
  call use_qhull()
  call clean_tempfile() !delete the used temp files. 
...
CONTAINS
  SUBROUTINE read_data()
    ...
    do i = 1, num_unitcell_points
       read(input_fileid, *) points(i, :)
       write(*,*) points(i,:) !output
    end do
  ...
  SUBROUTINE use_qhull()
    ...
    REAL, DIMENSION(:, :), ALLOCATABLE :: voronoi_vertices
